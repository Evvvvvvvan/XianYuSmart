...
const resourceTypes: Array<{ value: ResourceType; label: string; group: string }> = [
  { value: 'MATERIAL', label: '素材库', group: '商品运营' },
  { value: 'ADDRESS', label: '地址库', group: '商品运营' },
  { value: 'SUPPLY', label: '商品采集', group: '采集分销' },
  { value: 'SELECTION_RULE', label: '选品规则', group: '采集分销' },
  { value: 'PUBLISH_RULE', label: '发布规则', group: '任务规则' },
  { value: 'DELETE_RULE', label: '删除规则', group: '任务规则' },
  { value: 'PROMOTION_ACCOUNT', label: '返佣账号', group: '返佣运营' },
  { value: 'ANNOUNCEMENT', label: '公告管理', group: '服务管理' },
  { value: 'FEEDBACK', label: '系统反馈', group: '服务管理' },
  { value: 'RISK_EVENT', label: '风控记录', group: '服务管理' }
]

const activeView = ref<'resources' | 'tasks' | 'distributions'>('resources')
...
const switchView = async (view: 'resources' | 'tasks' | 'distributions') => {
...
<template>
  <div class="operations-page">
    ...
    <section v-if="activeView === 'resources'" class="workspace">
        ...
        <div class="table-scroll"><table>
          ...
          <tbody>
            <tr v-for="resource in resources" :key="resource.id">
              ...
              <td><strong>{{ resource.name }}</strong><small>#{{ resource.id }} · {{ resource.resourceType }}</small></td>
              ...
              <td class="actions">
                ...
                <button v-if="['SUPPLY','SELECTION_RULE','PUBLISH_RULE','DELETE_RULE','PROMOTION_ACCOUNT','MATERIAL'].includes(resource.resourceType)" @click="runResource(resource)">执行</button>
            ...
            <tr v-if="!loading && !resources.length"><td :colspan="activeType === 'MATERIAL' ? 7 : 6" class="empty">暂无数据，点击右上角创建</td></tr>
          </tbody>
        </table></div>
      </div>
    </section>
    ...
    <section v-else-if="activeView === 'tasks'" class="content-card full-card">
      ...
      <div class="table-scroll"><table><thead><tr><th>任务</th><th>资源</th><th>账号</th><th>状态</th><th>执行次数</th><th>计划时间</th><th>结果</th><th>操作</th></tr></thead><tbody>
        <tr v-for="task in tasks" :key="task.id"><td><strong>{{ task.taskType }}</strong><small>#{{ task.id }}</small></td><td>{{ task.resourceId ? `#${task.resourceId}` : '-' }}</td><td>{{ accountName(task.xianyuAccountId) }}</td><td><span class="status" :class="{ enabled: task.status === 2, failed: task.status === -1 }">{{ taskStatusText(task.status) }}</span></td><td>{{ task.attemptCount }}/{{ task.maxAttempts }}</td><td>{{ formatTime(task.scheduledTime) }}</td><td class="result-cell">{{ task.errorMessage || task.resultJson || '-' }}</td><td class="actions"><button v-if="task.status === -1" @click="retryTask(task)">重试</button></td></tr>
    ...
    <div v-if="showEditor" class="dialog-mask" @click.self="showEditor = false">
      <form class="editor" @submit.prevent="submitForm">
        <header><div><h2>{{ form.id ? '编辑' : '新建' }}{{ currentType.label }}</h2><p>仅填写当前业务需要的字段</p></div><button type="button" @click="showEditor = false">×</button></header>
        <div class="form-grid">
          ...
          <template v-if="form.resourceType === 'PUBLISH_RULE'"><label><span>素材 ID</span><input v-model.number="form.materialId" type="number" min="1"></label></template>
