...
const rateRuleMode = ref<0 | 1 | 2>(2)
...
const normalizeAutoRateMode = (mode: number): 0 | 1 | 2 => mode === 1 || mode === 2 ? mode : 0
...
const rateRuleError = computed(() => {
  ...
  if (rateRuleMode.value === 0) return ''
...
const openRateRuleDialog = async () => {
  ...
  rateRuleMode.value = currentGoods ? normalizeAutoRateMode(currentGoods.xianyuAutoRateOn) : 2
...
const saveRateRule = async () => {
  ...
  try {
    const content = rateRuleMode.value !== 0
    ...
    for (let index = 0; index < rateRuleGoodsIds.value.length; index += 200) {
      const response = await updateGoodsAutomationStatus({
        ...
        xianyuAutoRateOn: rateRuleMode.value,
    ...
    rateRuleGoods.value.filter(goods => rateRuleGoodsIds.value.includes(goods.item.xyGoodId)).forEach(goods => {
      goods.xianyuAutoRateOn = rateRuleMode.value
    ...
    <Transition name="overlay-fade">
      <div v-if="showRateRuleDialog" class="orders__dialog-overlay" @click.self="showRateRuleDialog = false">
        <form class="orders__dialog rate-dialog" @submit.prevent="saveRateRule">
          <div class="orders__dialog-header"><div><h3 class="orders__dialog-title">批量自动评价规则</h3><p>选择商品后统一设置触发方式和评价文案</p></div></div>
          <div class="orders__dialog-body rate-dialog__body">
            ...
            <section class="rate-dialog__section">
              ...
              <div class="rate-dialog__action-group">
                <button type="button" :class="{ active: rateRuleMode === 1 }" @click="rateRuleMode = 1"><strong>始终自动评价</strong><small>订单可评价时直接执行</small></button>
                <button type="button" :class="{ active: rateRuleMode === 2 }" @click="rateRuleMode = 2"><strong>买家评价后评价</strong><small>买家未评价时保持等待</small></button>
                <button type="button" :class="{ active: rateRuleMode === 0 }" @click="rateRuleMode = 0"><strong>关闭自动评价</strong><small>不再自动处理评价</small></button>
              </div>
              <div class="rate-dialog__trigger-note"><strong>当前规则</strong><span>{{ rateRuleMode === 1 ? '订单进入可评价状态后自动评价，不等待买家评价。' : rateRuleMode === 2 ? '仅在买家已经评价且卖家仍待评价时自动评价。' : '所选商品不会触发自动评价。' }}</span></div>
            </section>
            <section v-if="rateRuleMode !== 0" class="rate-dialog__section">
